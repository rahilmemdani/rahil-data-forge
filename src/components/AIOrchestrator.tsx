            ...
            <div className="container-custom px-4 relative z-10">
                ...
                <div className="grid lg:grid-cols-12 gap-10 items-start">
                    ...
                    <div className="lg:col-span-4 space-y-5">
                        {scenarios.map((s, idx) => (
                            ...
                            >
                                ...
                                <div className="flex items-start gap-4">
                                    <div className={`p-3 rounded-xl transition-colors duration-500 ${activeScenario.id === s.id ? 'bg-primary/10' : 'bg-muted group-hover:bg-primary/5'}`}>
                                        {s.icon}
                                    </div>
                                    <div>
                                        <h4 className={`font-bold text-base mb-1 transition-colors duration-300 ${activeScenario.id === s.id ? 'text-primary' : 'group-hover:text-primary'}`}>
                                            {s.title}
                                        </h4>
                        ...
                        <div className="p-8 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 dark:from-indigo-500/10 dark:to-violet-500/5 dark:border dark:border-white/10 text-white dark:text-foreground shadow-2xl mt-10 relative overflow-hidden group animate-fade-in-up" style={{ animationDelay: '450ms' }}>
                            ...
                            <div className="flex items-center gap-2 text-white/70 dark:text-primary text-[10px] font-mono mb-4 uppercase tracking-[0.2em]">
                                <Zap size={14} className="animate-pulse" /> System Impact
                            </div>
                            <p className="text-sm sm:text-base leading-relaxed relative z-10 font-medium italic opacity-90">
                    ...
                    <div className="lg:col-span-8 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                        <div className="rounded-3xl border border-border/40 bg-[#0B0E14] shadow-2xl overflow-hidden min-h-[520px] flex flex-col group relative">
                            {/* Subtle editor glow */}
                            ...
                            <div className="flex items-center justify-between px-6 py-5 border-b border-white/5 bg-white/[0.02] relative z-10">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center gap-2 text-[10px] sm:text-xs font-mono text-white/30 uppercase tracking-[0.2em] font-medium">
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-5 py-2.5 bg-primary hover:bg-primary/90 disabled:bg-white/5 text-primary-foreground text-xs font-bold rounded-xl transition-all shadow-xl shadow-primary/10 active:scale-95 disabled:opacity-50"
                                >
                            ...
                            <div className="flex-1 p-8 relative overflow-hidden z-10">
                                <pre className="font-mono text-sm sm:text-base leading-relaxed text-sky-200/90 whitespace-pre-wrap selection:bg-primary/30">
                                    {activeScenario.code.split('\n').map((line, i) => (
                                        <div key={i} className="flex gap-6 group/line">
                                            <span className="w-6 text-right text-white/10 select-none text-xs leading-relaxed group-hover/line:text-white/30 transition-colors">{i + 1}</span>
                                ...
                                <AnimatePresence>
                                    {showResult && (
                                        <motion.div
                                            ...
                                            className="absolute inset-x-8 bottom-8 p-6 rounded-2xl bg-[#161B22]/95 backdrop-blur-2xl border border-white/10 shadow-3xl z-20 overflow-hidden"
                                        >
                                            ...
                                            <div className="overflow-x-auto relative z-10">
                                                <table className="w-full text-left font-mono text-[10px] sm:text-xs text-white/80">
                                                    <thead>
                                                        <tr className="border-b border-white/10 opacity-60">
                                                            {Object.keys(activeScenario.output[0]).map(key => (
                                                                <th key={key} className="pb-3 px-2 font-bold uppercase tracking-widest text-primary/80">{key}</th>
                                                            ))}
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {activeScenario.output.map((row, i) => (
                                                            <tr key={i} className="border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-colors rounded-lg">
                            ...
                            {/* Editor Footer */}
                            <div className="px-6 py-4 border-t border-white/5 bg-black/40 text-[10px] text-white/20 font-mono flex gap-8 relative z-10">
                                <span className="flex items-center gap-1.5"><Terminal size={12} /> /usr/bin/python3</span>
                                <span className="flex items-center gap-1.5">UTF-8</span>
                                <span className="flex items-center gap-1.5 ml-auto">RAG / Agentic Orchestration</span>
                            </div>
                        </div>
                    </div>
                </div>
